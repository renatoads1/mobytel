...
//                Response()->not_authorized();
            ...
            if(isset($id) && !$current_user->can('read', $model)){
                Response()->not_authorized();
            }
        }
        ...
        if (@func_get_arg(1) == false) {
            ...
            //if we're not getting a specific model
            if ($id == null){
                Response($model->get());
            }
            else {
                //if we are getting a specific model
    ...
    function delete($id = null)
    {
        ...
        //if we are requesting a specific model, make sure the current user is the owner, if we're not getting a specific model
        //and are getting a list, the list will be filtered in each of the individual models
        if($this->check_authorization('delete', $model)){
            $result = $model->delete();
            Response($result);
        }
    }
    ...
    function save()
    {
        ...
        if(isset($id) && $model->is_valid())
            $this->check_authorization('update', $model);
        else $this->check_authorization('create', $model);
    ...
    function check_authorization($action, Model $model){
    ...
    function logged_in()
    {
        ...
        if (!$this->user)
            Response()->not_logged_in();
        else Response()->logged_in(array('rights'=>RBAC::export()));
    }
    ...
    function messages($reference_object, $reference_id)
    {
        ...
        $this->check_authorization('read', $model);
