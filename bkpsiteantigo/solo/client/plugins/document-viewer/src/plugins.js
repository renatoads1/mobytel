...
//TODO:Tinyscrollbar is currently being included as a part of the docuemnt viewer, but it should be a standalone plugin in the plugins folder since it is used by other parts of the applicaiton besides the document viewer
(function($) {
    $.tiny = $.tiny || { };
    ...
    function Scrollbar(root, options) {
        ...
        function initialize() {
            ...
            oSelf.update();
            setEvents();
            //SE: Add class to viewport if we are at the top of the content
            oViewport.obj.addClass('top');
            return oSelf;
        }
        ...
        function wheel(oEvent) {
            if (!(oContent.ratio >= 1)) {
                ...
                if(iScroll === 0)
                    ...
                    if(oViewport.obj.hasClass('top'))
                        oViewport.obj.removeClass('top');
                }
...
(function($) {
    //from http://stackoverflow.com/questions/18082/validate-numbers-in-javascript-isnumeric (CMS answer)
    function isNumber(n) {
        ...
        this.each(function() {
            if (width) {
                ...
                if(!$this.borderBoxSizing())
                    ...
                    //since it's possible for padding to evaluate to NaN
    ...
    $.fn.borderBoxSizing = function(){
            var $body = $('body'),
                boxSizing;

        boxSizing = $body.css('box-sizing') ||
                $body.css('moz-box-sizing') ||
                $body.css('webkit-box-sizing');
